...
void graph_traverse_recursive(GraphNode *node, GraphNode **visited_nodes, GraphCallback callback,
                              GraphCallbackArg *arg) {
  if (arg->debug == true) arg->counter++;

  if (node == NULL || visited_nodes[node->id] == node) return;
...
GraphNode *graph_node_find_recursive(GraphNode *node, unsigned int nodeid,
  ...
  if (node == NULL || visited_nodes[node->id] == node) return NULL;
